...
{
    /// <summary>
    /// Provides cutting logic for platforms, creating the fallen part and adjusting the overlap.
    /// </summary>
    public static class CutEngine
    {
        public static CutPlatformResult CutPlatform(
            ...
            Vector3 initialScale)
        {
            ...
            {
                GameObject cuttingPlatformMock = Object.Instantiate(
                    cuttingPlatform.gameObject,
                    cuttingPlatform.transform.position,
                    Quaternion.identity);
                cuttingPlatform.gameObject.SetActive(false);
                float torqueRandomness = 20;
                cuttingPlatformMock.AddComponent<Rigidbody>().AddTorque(
                    new Vector3(
                        Random.Range(-torqueRandomness, torqueRandomness),
                        Random.Range(-torqueRandomness, torqueRandomness),
                        Random.Range(-torqueRandomness, torqueRandomness)));
            ...
            GameObject fallenPart = CreateFallenPart(
                initialScale,
                xDif,
                overlapWidth,
                cuttingPlatform.transform.position,
                cuttingPlatform.meshRenderer.material);
            cuttingPlatform.ResizeCollider();
            return new CutPlatformResult(true, fallenPart, cuttingPlatform, xDif);
        }

        private static GameObject CreateFallenPart(
            Vector3 baseScale,
            float xDif,
            float overlapWidth,
